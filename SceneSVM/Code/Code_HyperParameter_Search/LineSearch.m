...
nel = numel(err_grid);
coordsTemplate = cell(1,ndims(err_grid));
...
while(changed)
    ...
    parfor ii=1:nel
        ...
        if (sum([coords{:}] == positioni) >= (pdim-1)) && (err_grid(ii) == bitmax)
            ...
            MP_accum = zeros(KFolds,1);
            MR_accum = zeros(KFolds,1);
            for k = 1:KFolds
                test = (kfold_indices == k); train = ~test;
                [kfold_accum(k),~,~,MP_accum(k),MR_accum(k)] = GetPerformance(data(train,:),labels(train),data(test,:),labels(test), paramsLocal, passthrough);
            end
